...
## Writing generic functions for ploting the summary
...
# now combine this with original data set to plot values:
  ...
  ggplot(new |> mutate(year = as.numeric(year)), aes(x = year, y = get(var_name), colour = area_name)) + 
    ...
    guides(linetype = "none") +
    labs(x = "Year", y = "Enrollment mean", title = "Enrollment changes over the years", colour = "County")
...
plot.county(state = "NC", 
            data = final_combined_data$Combined_County_Data, 
            var_name = "enrollment", 
            interest = "top", 
            x = 15)
```
...
We will use our data processing function on two enrollment URLs:
```{r}
...
We will now run our data combining function to put these into one object (with two data frames):
...
We will now use the plot function on the state data frame
```{r}
plot.state(data = final_combined_data$Combined_State_Data, 
           var_name = "enrollment")
```


We will now use the plot function on the county data frame.
...
```{r}
plot.county(data = final_combined_data$Combined_County_Data )
```
...
plot.county(state = "PA", 
            ...
            x = 8)
```
Now we will read in four more data sets and apply our functions:
```{r}
